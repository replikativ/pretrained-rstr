...
(defn- parse-merges
  ...
  [left right] -> rank (index). Handles BOTH serializations: legacy strings
  (\"le ft\") and the newer tokenizers array-of-pairs ([\"le\" \"ft\"], used
  by e.g. Qwen3)."
  [merges-list]
  (let [m (HashMap. (count merges-list))]
    (dotimes [i (count merges-list)]
      (let [entry (nth merges-list i)]
        (if (vector? entry)
          (.put m [(nth entry 0) (nth entry 1)] (int i))
          (let [space-idx (.indexOf ^String entry " ")]
            (when (pos? space-idx)
              (.put m [(.substring ^String entry 0 space-idx)
                       (.substring ^String entry (inc space-idx))]
                    (int i)))))))
    m))
