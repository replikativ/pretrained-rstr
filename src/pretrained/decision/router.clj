...
(defn- close-agent! [agent]
  (when (instance? java.io.Closeable agent)
...
(defn router
  ...
  :lang-guess, :decision-opts (e.g. {:gpu? true}), :loader (useful for local
  checkpoints/tests), and :preload.
  ...
  ([{:keys [max-loaded default auto-task-detection? loader preload lang-guess
            decision-opts]
     :or {max-loaded 2 default :english}}]
   (let [loader (or loader
                    (if (seq decision-opts)
                      #(decision/load-decision % decision-opts)
                      decision/load-decision))
         router (->LayaRouter (atom {}) (atom [])
...
(defn- evict! [router]
  (while (> (count @(:order router)) @(:max-loaded router))
    (let [victim (first @(:order router))
          agent (get @(:agents router) victim)]
...
(defn attach!
  ...
  (locking router
    (let [key (normalize-name name)]
      (when-let [previous (get @(:agents router) key)]
        (when-not (identical? previous agent)
          (close-agent! previous)))
...
(defn unload!
  ([router]
   (locking router
     (doseq [agent (vals @(:agents router))] (close-agent! agent))
     (reset! (:agents router) {})
     (reset! (:order router) [])
     router))
  ([router name]
   (locking router
     (let [key (normalize-name name)]
       (close-agent! (get @(:agents router) key))
