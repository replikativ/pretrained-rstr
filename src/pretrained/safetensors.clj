...
(defn- read-tensor-floats
  ...
  (let [offset (long offset) end-offset (long end-offset) numel (long numel)
    ...
    (case dtype
      ...
      ;; F16/BF16: bulk-read shorts in chunks + tight primitive loop — per-element
      ;; .getShort is ~10x slower (virtual call + position bump each); a full-size
      ;; temp array doubles peak allocation (GC thrash on big embed tables).
      "F16" (let [sb (.asShortBuffer buf) chunk (short-array 262144)]
              (loop [off 0]
                (when (< off numel)
                  (let [n (int (min 262144 (- numel off)))]
                    (.get sb chunk 0 n)
                    (dotimes [j n]
                      (aset out (+ off j) (Float/float16ToFloat (aget chunk j))))
                    (recur (+ off n))))))
      "BF16" (let [sb (.asShortBuffer buf) chunk (short-array 262144)]
               (loop [off 0]
                 (when (< off numel)
                   (let [n (int (min 262144 (- numel off)))]
                     (.get sb chunk 0 n)
                     (dotimes [j n]
                       ;; bit ops widen to long; unchecked-int keeps the 32-bit pattern
                       (aset out (+ off j)
                             (Float/intBitsToFloat
                              (unchecked-int (bit-shift-left (bit-and (int (aget chunk j)) 0xFFFF) 16)))))
                     (recur (+ off n))))))
...
(defn read-tensor
  ...
  (let [{:keys [path data-offset tensors]} lazy-st
    ...
    (with-open [raf (RandomAccessFile. ^String path "r")
                channel (.getChannel raf)]
      ;; map ONLY this tensor's slice — whole-region maps break >2GB shards
      ;; (MappedByteBuffer is Integer.MAX_VALUE-limited)
      (let [len (- (long end-offset) (long offset))
            mmap (.map channel FileChannel$MapMode/READ_ONLY
                       (+ (long data-offset) (long offset)) len)]
        {:dtype dtype :shape shape
         :data (read-tensor-floats mmap 0 len dtype numel)}))))
